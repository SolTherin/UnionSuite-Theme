...
| `refreshQueryTemplate(selector,opts)` | Available only if the site's legacy script supplies it. Custom action refresh callbacks can call it; it is not promoted to a universal theme reload method. | Reject non-OK/login/wrong-page responses, validate target identities before replacement, reject stale concurrent responses, preserve focus/filter state and use explicit supported initialisers. Verify each fragment type. |
