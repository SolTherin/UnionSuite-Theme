...
- `../Theme-Config.html`: dedicated, offline client branding workspace at the
  ...
  `../../../tools/build-theme-config.cjs` reuses the guide builder's canonical
...
- `../../../tools/build-theme-usage.cjs`: dependency-free Node generator.
- `../../../prototypes/List-Templates/*.html`: canonical bulletin, member-note,
  task-row, history, no-shell, native-bulletin and empty-state HTML examples.
  `../../../tools/list-template-examples.cjs` builds their guide preview and
...
- `../../../tools/theme-gallery.cjs`: visual cards, class chips and code blocks
...
- `../../../prototypes/Banner-*.html`: banner templates and the legacy-named
  generated compatibility assets. No independent CSS/JS edits belong in those assets.
...
node tools/check-theme-config.cjs
```

For config interaction and desktop/mobile visual checks, run
`node tools/test-theme-config.cjs` with local Microsoft Edge available (or set
