...
Build: `node tools/build-membership-stats.cjs`, `node tools/build-home-preview.cjs`, then `node tools/build-theme-usage.cjs`. Run each with `--check` for freshness. Browser checks: `node tools/test-membership-stats.cjs`, `node tools/test-membership-layout.cjs` and `node tools/test-home-stats.cjs`.
