...
See CCO-Retained-Tabs-Trial.md (archived locally: `archive/Custom CCO iPart/research/CCO-Retained-Tabs-Trial.md`) and its script. Reads native labels, uses published same-page Directory/TemplateType URLs, forwards parent query parameters and bridges child popups to the outer helper. Supersedes fixed preview-GUID mappings for the next experiment. Add Job width and child save refresh were confirmed by the user in the separate About-frame test; the combined implementation still needs live validation.
