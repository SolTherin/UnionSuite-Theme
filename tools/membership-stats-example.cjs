...
const folder = 'prototypes/Home/Stats/';
...
exports.sources = ['tools/membership-stats-example.cjs','prototypes/Home/Stats-Content.html','prototypes/Home/home-stats.css','prototypes/Home/home-stats.js',folder+'captured-data.json',folder+'README.md',...Object.values(cards).map(file => folder+file)];
exports.card = key => read(folder+cards[key]).trim();
exports.composition = () => Object.keys(cards).reduce((html,key) => html.replace('{{STATS_'+key.toUpperCase()+'}}',exports.card(key)),read('prototypes/Home/Stats-Content.html'));
...
exports.fixture = () => read('prototypes/Home/home-stats.js').replace('{{MEMBERSHIP_SNAPSHOT}}',JSON.stringify(JSON.parse(read(folder+'captured-data.json'))).replace(/</g,'\\u003c'));
