...
const bulletinTemplate = read('prototypes/List-Templates/Bulletin-Query-Template.html');
records.splice(3);
...
const html = read('prototypes/Home/Home-Preview.html').replace('{{WELCOME}}',read('prototypes/Home/Welcome-Content.html').trim()).replace('{{ATTENTION}}',read('prototypes/Home/Needs-Attention-Content.html').trim()).replaceAll('{{CHEVRON}}',chevron).replace('{{TASKS}}',tasks.filter(task=>!task.done).map(taskRow).join('')).replace('{{BULLETIN}}',bulletin).replace('{{STATS}}',membership.composition());
...
${read('prototypes/Home/home-stats.css')}</style></head><body class="home-preview" data-attention-example-count="3">${html}<script>${read('THeme/UnionSuite/docs/attention-example.js')}\n${actions}\n${actionIcons}\n${attention}\n${js}
