...
const sources = ['tools/query-empty-example.cjs','prototypes/List-Templates/Contacts-Query-Template.html','THeme/UnionSuite/docs/contact-records.cjs'];
function body(results) {
  if (!results) return '<p>No delegates found.</p>';
  const template = read('prototypes/List-Templates/Contacts-Query-Template.html').replace(/<!--[\s\S]*?-->/g,'');
