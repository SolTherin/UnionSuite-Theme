...
const base = 'prototypes/Action-Builder/';
const cleanCss = source => source
  .replace(/@import\s+[^;]+;/g, '')
  .replace(/@font-face\s*\{[^}]*\}/g, '')
  .replace(/url\((?!["']?data:)[^)]*\)/g, 'none');
const native = cleanCss(read('Native CSS/10-UltraWaveResponsive.css') + '\n' + read('THeme/UnionSuite/99-Orion.css'));
...
const options = [{label: 'None', value: ''}, ...require('../THeme/UnionSuite/docs/action-catalog.cjs').actions.map(item => ({
...
const sources = [
  'tools/build-action-builder.cjs',
