...
(async () => {
  ...
  try {
    for (const mobile of [false, true]) {
      ...
      console.log(base, await pip.evaluate(el => ['button', '.pip-ear-left', '.pip-ear-right'].map(selector => {const e=el.querySelector(selector); return {selector,rect:e.getBoundingClientRect().toJSON(),transform:getComputedStyle(e).transform};})));
      assert(await pip.evaluate(el => {
