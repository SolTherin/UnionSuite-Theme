...
const script = require('../tools/theme-sources.cjs').bannerBehaviour(themeJs);
...
Do not add CSS here. Rebuild with node .preview/build-banner-preview.cjs.
-->
<script id="us-banner-behaviour">
${script}
</script>
`;
fs.writeFileSync(path.join(root, 'prototypes/Banner-Shared-Styles.html'), embed);
const template = read('prototypes/Banner-Template.html');
fs.writeFileSync(path.join(root, 'prototypes/Banner-Behaviour.js'), script.trim() + '\n');
const css = ['Native CSS/10-UltraWaveResponsive.css', 'THeme/UnionSuite/99-Orion.css', 'THeme/UnionSuite/zUnionSuite.css']
  .map(read).join('\n').replace(/@import\s+[^;]+;/g, '');
const values = {
...
<!-- Generated preview. Edit zUnionSuite.css / zUnionSuite.js / Banner-Template.html,
     then run node .preview/build-banner-preview.cjs. Sample data only. -->
